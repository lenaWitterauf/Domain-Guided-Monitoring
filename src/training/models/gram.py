...
class GramEmbedding(tf.keras.Model, BaseEmbedding):
    def __init__(self, hierarchy: HierarchyKnowledge, config: ModelConfig):
        ...
        self._init_connection_information(hierarchy)
    ...
    def _init_basic_embedding_variables(self, hierarchy: HierarchyKnowledge):
        ...
        self.basic_hidden_embeddings = self.add_weight(
            initializer=self._get_hidden_initializer(
                self._load_description_vocab(
                    hierarchy, set(hierarchy.extra_vocab.values())
                )
            ),
    ...
    def _load_description_vocab(
        self, hierarchy: HierarchyKnowledge, ids: Set[int]
    ) -> Dict[int, str]:
        return {
            idx: node.label_name for idx, node in hierarchy.nodes.items() if idx in ids
        }

    def _init_connection_information(self, hierarchy: HierarchyKnowledge):
        logging.info("Initializing GRAM connection information")
        ...
        for idx in tqdm(
            range(self.num_features), desc="Initializing GRAM connections",
        ):
            node = hierarchy.nodes[idx]
            connected_idxs = set(node.get_ancestor_label_idxs() + [idx])
