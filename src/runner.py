...
class ExperimentRunner:
    ...
    def _generate_artifacts(self, metadata: sequences.SequenceMetadata, knowledge: Any, model: models.BaseModel):
    ...
    def _create_dataset(self, sequence_df: pd.DataFrame) -> Tuple[tf.data.Dataset, tf.data.Dataset]:
        if self.config.use_dataset_generator:
            ...
            ).batch(self.config.batch_size).prefetch(
                tf.data.experimental.AUTOTUNE
            ).cache(
                self._get_cache_file_name(is_test=False)
            ).shuffle(self.config.dataset_shuffle_buffer, seed=self.config.dataset_shuffle_seed, reshuffle_each_iteration=True)
            ...
            train_dataset = tf.data.Dataset.from_tensor_slices(
                ...
                tf.data.experimental.AUTOTUNE
            ).cache().shuffle(self.config.dataset_shuffle_buffer, seed=self.config.dataset_shuffle_seed, reshuffle_each_iteration=True)
            test_dataset = tf.data.Dataset.from_tensor_slices(
                (split.test_x, split.test_y),
            ).batch(self.config.batch_size).prefetch(tf.data.experimental.AUTOTUNE).cache()
    ...
    def _get_cache_file_name(self, is_test: bool) -> str:
        if len(self.config.dataset_generator_cache_file) < 1:
            ...
            return self.config.dataset_generator_cache_file + ('_test' if is_test else '_train')
    ...
    def _load_model(self, metadata: sequences.SequenceMetadata) -> Tuple[Any, models.BaseModel]:
        ...
        if self.config.model_type == 'simple':
            ...
            raise InputError(message='Unknown model type: ' + str(self.config.model_type))
    ...
    def _load_description_knowledge(self, metadata: sequences.SequenceMetadata) -> knowledge.DescriptionKnowledge:
        ...
        if self.config.sequence_type == 'mimic':
            ...
            logging.fatal('Description knowledge not available for data type %s', self.config.sequence_type)
            raise InputError(message='Description knowledge not available for data type: ' + str(self.config.sequence_type))
    ...
    def _load_causal_knowledge(self, metadata: sequences.SequenceMetadata) -> knowledge.CausalityKnowledge:
        ...
        if self.config.sequence_type == 'huawei_logs':
            ...
            logging.fatal('Causal knowledge not available for data type %s', self.config.sequence_type)
            raise InputError(message='Causal knowledge not available for data type: ' + str(self.config.sequence_type))
    ...
    def _load_hierarchy_knowledge(self, metadata: sequences.SequenceMetadata) -> knowledge.HierarchyKnowledge:
        ...
        if self.config.sequence_type == 'mimic':
            ...
            logging.fatal('Hierarchy knowledge not available for data type %s', self.config.sequence_type)
            raise InputError(message='Hierarchy knowledge not available for data type: ' + str(self.config.sequence_type))
    ...
    def _load_sequences(self) -> pd.DataFrame:
        ...
        if self.config.sequence_type == 'mimic':
            ...
            raise InputError(message='Unknown data type: ' + str(self.config.sequence_type))

    def _collect_sequence_metadata(self, sequence_df: pd.DataFrame) -> sequences.SequenceMetadata:
        if self.config.max_data_size > 0:
            logging.debug('Using subset of length %d instead total df of length %d', self.config.max_data_size, len(sequence_df))
            sequence_df = sequence_df[0:self.config.max_data_size]
