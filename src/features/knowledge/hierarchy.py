...
class HierarchyKnowledge:
    def __init__(self, child_id_col='child_id', parent_id_col='parent_id', child_name_col='child_name', parent_name_col='parent_name'):
    ...
    def _build_extended_vocab(self, hierarchy_df: pd.DataFrame, vocab: Dict[str, int]):
        ...
        while len(labels_to_handle) > 0:
            ...
            label_names = set(hierarchy_df[hierarchy_df[self.child_id_col] == label][self.child_name_col])
            label_names.update(set(hierarchy_df[hierarchy_df[self.parent_id_col] == label][self.parent_name_col]))
